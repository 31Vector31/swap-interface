...
export default function ChainSelectorRow({ disabled, targetChain, onSelectChain, isPending, isActive }: ChainSelectorRowProps) {
  ...
  const logoUrl = chainInfo?.logoUrl*/
    ...
    const label = targetChain;

  const theme = useTheme()

  return (
    <TraceEvent events={[BrowserEvent.onClick]} name={SharedEventName.ELEMENT_CLICKED}>
      <Container
        disabled={!!disabled}
        onClick={() => {
      ...
      >
        ...
        <Status>
          {isActive && <CheckMarkIcon width={LOGO_SIZE} height={LOGO_SIZE} color={theme.accent1} />}
